...
pcl::visualization::PCLVisualizer::Ptr initScene(Box window, int zoom)
{
    ...
    viewer->addLine(pcl::PointXYZ(window.x_max, window.y_max, 0),pcl::PointXYZ(window.x_min, window.y_max, 0),1,1,1,"top");
    viewer->addLine(pcl::PointXYZ(window.x_max, window.y_min, 0),pcl::PointXYZ(window.x_min, window.y_min, 0),1,1,1,"bottom");
    viewer->addLine(pcl::PointXYZ(window.x_max, window.y_max, 0),pcl::PointXYZ(window.x_max, window.y_min, 0),1,1,1,"right");
    viewer->addLine(pcl::PointXYZ(window.x_min, window.y_max, 0),pcl::PointXYZ(window.x_min, window.y_min, 0),1,1,1,"left");
  	return viewer;
}
...
void proximity(const std::vector<std::vector<float>> points,std::vector<bool>& processed,std::vector<int>& cluster, KdTree* tree, float distanceTol,int index)
{
    ...
    {
        if(!processed[id])
            proximity(points,processed,cluster,tree,distanceTol,id);
    }
}
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>> points, KdTree* tree, float distanceTol)
{
    ...
    for(int index = 0; index < points.size(); index++)
    {
        if(!processed[index])
        {
            ...
            proximity(points,processed,cluster,tree,distanceTol,index);
